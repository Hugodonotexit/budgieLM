...
def _make_date_lines() -> list[str]:
    return [f"DATE: 2026-02-{d:02d}\n" for d in range(1, 29)]
...
def eval_next_line(
    ...
    *,
    ...
    if not (0 < prompt_lines < len(lines)):
        raise ValueError(f"`prompt_lines` must be in [1, {len(lines)-1}], got {prompt_lines}.")

    prompt = "".join(lines[:prompt_lines])
    expected = lines[prompt_lines]
    prompt_ids = tok.encode(prompt, add_bos=True, add_eos=False)
    expected_ids = tok.encode(expected, add_bos=False, add_eos=False)

    inp = torch.tensor([prompt_ids], dtype=torch.long, device=device)
    gen = greedy_decode(model, inp, max_new_tokens=len(expected_ids), amp=amp)
    got_ids = gen[0, len(prompt_ids) :].tolist()

    got = tok.decode(got_ids)
    exact = got == expected
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Budgie tiny AMP pattern-learning test (date continuation).")
    ...
    parser.add_argument("--repeat", type=int, default=200, help="How many times to repeat the date block.")
    parser.add_argument("--prompt_lines", type=int, default=10)
    ...
    base_text = "".join(_make_date_lines())
    train_text = base_text * int(args.repeat)
    ...
    config = BudgieConfig(
        ...
        max_position_embeddings=max(256, args.seq_len + 128),
    ...
    for step in range(1, int(args.steps) + 1):
        ...
        if args.eval_every and step % int(args.eval_every) == 0:
            ok = eval_next_line(
                ...
                device=device,
                amp=amp,
                prompt_lines=int(args.prompt_lines),
            )
            model.train()
            if ok:
                print("Solved (exact match). Stopping early.")
                break
